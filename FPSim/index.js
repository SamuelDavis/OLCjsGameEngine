...
new class extends GameEngine {
    constructor() {
        ...
        this.player = new Player(3.0, 3.0, 1.57);
    ...
    init() {
        ...
        window.addEventListener('keydown', (e) => {
            if (e.key === 'i')
                this.debug.style.display = this.showingDebug ? 'none' : 'block';
        });
    ...
    renderWorld() {
        ...
        for (let x = 0; x < this.width; x++) {
            const fRayAngle = (this.player.a - this.HALF_FOV) + (x / this.width) * this.FOV;
